...
// keep a handle to the original console
export const directConsoleAccess: Console = console
...
// Implementation (must be the single definition with a body)
  ...
  public timeStamp: ((label?: string) => void) = directConsoleAccess.timeStamp;
  public profile: ((label?: string) => void) = directConsoleAccess.profile;
  public profileEnd: ((label?: string) => void) = directConsoleAccess.profileEnd;
}

// Assign into the global console. Cast through `unknown` to avoid `any`.
const replacement = Object.freeze(new CustomConsole()) as unknown as Console;
console = replacement;
